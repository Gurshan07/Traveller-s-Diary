...
import { Loader2, Calendar, Clock, Sword, Skull, Star, ChevronDown, ChevronUp, Target } from 'lucide-react';
import { ELEMENT_ICONS } from '../constants';

interface OnslaughtPageProps {
  data: UserData;
}
        ...
        <div className="bg-white/70 dark:bg-slate-800/60 backdrop-blur-lg rounded-2xl shadow-sm border border-white/50 dark:border-white/10 overflow-hidden hover:shadow-md transition-all">
            ...
            {isOpen && (
                <div className="p-4 pt-0 border-t border-slate-200/50 dark:border-slate-700/50 bg-slate-50/30 dark:bg-slate-900/30">
                     ...
                     {stage.best_avatar && stage.best_avatar.length > 0 && (
                        <div className="mt-4 pt-4 border-t border-slate-200/50 dark:border-slate-700/50">
                            <h4 className="text-xs font-bold uppercase text-slate-500 dark:text-slate-400 mb-2 flex items-center gap-1.5">
                                <Target size={14} /> Performance Highlights
                            </h4>
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                {stage.best_avatar.map((ba, idx) => (
                                    <div key={idx} className="flex items-center gap-3 bg-white/50 dark:bg-slate-800/50 rounded-lg p-3 border border-slate-100 dark:border-slate-700/50">
                                        <div className="w-10 h-10 shrink-0 bg-slate-200 dark:bg-slate-700 rounded-full overflow-hidden">
                                            <img src={ba.side_icon} alt="Avatar" className="w-full h-full object-cover" referrerPolicy="no-referrer" />
                                        </div>
                                        <div>
                                            <p className="text-[10px] text-slate-500 uppercase font-bold">
                                                {ba.type === 1 ? 'Strongest Strike' : 'Total Damage'}
                                            </p>
                                            <p className="font-mono font-bold text-slate-800 dark:text-slate-200 text-lg">
                                                {parseInt(ba.dps).toLocaleString()}
                                            </p>
                                        </div>
                                    </div>
                                ))}
...
const OnslaughtPage: React.FC<OnslaughtPageProps> = ({ data }) => {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        console.error(err);
        setError("Failed to load Onslaught data.");
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="space-y-6 pb-20">
       <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
          <div>
            <h2 className="text-3xl font-bold text-slate-800 dark:text-slate-100 tracking-tight">Onslaught</h2>
            <p className="text-slate-500 dark:text-slate-400">Limited-Time Combat Events</p>
          </div>
       ...
       {loading ? (
           ...
           <div className="p-12 text-center bg-white/30 dark:bg-slate-800/30 rounded-2xl border border-dashed border-slate-300 dark:border-slate-700 text-slate-500 dark:text-slate-400">
               <p>No Onslaught records found.</p>
           </div>
       ) : (
           <div className="animate-in fade-in duration-500 space-y-6">
               ...
               <div className="bg-[#1e1b29] dark:bg-slate-900 rounded-3xl p-6 md:p-8 text-white relative overflow-hidden shadow-xl border border-white/10">
                   ...
                   <div className="relative z-10">
                       <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
                           <div>
                                <span className="text-purple-300 text-xs font-bold uppercase tracking-widest mb-1 block">Active Event</span>
                       ...
                       {activeSeason.mp && activeSeason.mp.has_data ? (
                           <div className="flex flex-col sm:flex-row gap-4">
                                <div className="bg-white/10 backdrop-blur-md rounded-2xl p-4 flex-1 border border-white/10">
                                    <p className="text-xs text-slate-300 uppercase font-bold mb-1">Best Score / Difficulty</p>
                                    <div className="flex items-center gap-3">
                                         ...
                                         <div>
                                             ...
                                             <p className="text-xs text-slate-400">Highest Tier</p>
                                         </div>
                                    </div>
                                </div>
                                <div className="bg-white/10 backdrop-blur-md rounded-2xl p-4 flex-1 border border-white/10">
               ...
               {activeSeason.mp && activeSeason.mp.challenge && activeSeason.mp.challenge.length > 0 && (
                   <div className="space-y-4">
                       <h3 className="text-lg font-bold text-slate-800 dark:text-slate-100 px-1">Battle Records</h3>
